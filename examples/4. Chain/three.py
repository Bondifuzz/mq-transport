...
class MC_Chain(Consumer):
    ...
    async def consume(self, msg: Model, mq_app: MQApp):
        self._logger.info("[TWO->THREE] Consumed message: rnd=%d", msg.rnd)
...
class MQAppInitializer:
    ...
    async def _configure_input_channels(self):
        ...
        #
        # Create channel to read from queue 'two-three'
        # Add consumer to this channel
        #

        queues = self._settings.message_queue.queues
        ich1 = await self._app.create_consuming_channel(queues.two_three)
